...
const f = async () => {
  ...
  for (const deepPackage of packages) {
    // Remove old package
    const resultRemovePackage = await removePackage({
      deep,
      packageName: deepPackage.name,
    });
    ...
    const packageVersion = deepPackage.versionUpdate
      ? incrementPatchVersion(lastPackageVersion)
      : lastPackageVersion;
    log(`Package ${packageName} version is #️⃣${packageVersion}`);
    log("Generating package 📦 and namespace 🎁");
    const { packageId, packageNamespaceId } = await createEmptyPackage({
      deep,
      Types,
      packageName,
      packageVersion,
