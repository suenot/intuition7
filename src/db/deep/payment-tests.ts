...
export const createPaymentTests = async ({
  deep,
  Types,
  packageName,
  packageId,
  ...
  const {
    ...
    string: {
      data: {
        value:
          "https://w7.pngwing.com/pngs/305/230/png-transparent-shiba-inu-dogecoin-akita-cryptocurrency-bitcoin-mammal-cat-like-mammal-carnivoran-thumbnail.png",
      },
    },
  });
  ...
  const {
    ...
    string: {
      data: {
        value:
          'Dogecoin is a popular cryptocurrency that started as a playful meme in 2013. It features the Shiba Inu dog from the "Doge" internet meme as its console.logo. Despite its humorous origins, Dogecoin has gained a dedicated following and is used for tipping content creators, charitable donations, and as a digital currency for various online payments. It distinguishes itself with a vibrant and welcoming community and relatively low payment fees.',
      },
    },
  });
  ...
  const {
    ...
    string: {
      data: {
        value:
          "Buying toys for cats is essential for their physical and mental well-being, and it enhances the quality of their life while fostering a closer connection with their human companions.",
      },
    },
  });
  ...
  const {
    ...
    string: {
      data: {
        value:
          "https://github.com/suenot/portfolio-icons/blob/main/cat.jpeg?raw=true",
      },
    },
  });
  ...
  // Payments symbols
  const {
    data: [{ id: dependencePaymentsSymbols }],
  } = await deep.insert({
    type_id: ContainId,
    from_id: packageId,
  ...
  return {
    ...
    WalletUiPackageId,
    PaymentUiPackageId,
    PaymentsSymbolsPackageId,
    dependenceUnitUi,
    dependenceWalletUi,
    dependencePaymentUi,
    dependencePaymentsSymbols,
  };
};
